...
if [[ "$IORUNTIME_MODE" == "docker" ]]; then
    SERVERS+=("iouring|docker|native runtime http server (dockerized)")
else
    SERVERS+=("iouring|local|native runtime http server")
fi
SERVERS+=(
...
start_nginx() {
    ...
    docker run --rm -d \
        --name "$CURRENT_CONTAINER" \
        -p "${PORT}:8080" \
        -v "${ROOT_DIR}/benchmarks/wrk/reference_servers/nginx.conf:/etc/nginx/nginx.conf:ro" \
        -v "${ROOT_DIR}/benchmarks/wrk/reference_servers/www:/srv/www:ro" \
...
start_caddy() {
    ...
    docker run --rm -d \
        --name "$CURRENT_CONTAINER" \
        -p "${PORT}:8080" \
        -v "${ROOT_DIR}/benchmarks/wrk/reference_servers/Caddyfile:/etc/caddy/Caddyfile:ro" \
        -v "${ROOT_DIR}/benchmarks/wrk/reference_servers/www:/srv/www:ro" \
...
if [[ "$IORUNTIME_MODE" == "docker" ]]; then
    ...
    docker build \
        -f "${ROOT_DIR}/benchmarks/wrk/reference_servers/iouring.Dockerfile" \
...
for server_entry in "${SERVERS[@]}"; do
    ...
    case "$name" in
        iouring)
            if [[ "$kind" == "docker" ]]; then
                start_iouring_docker "$server_dir"
            else
                start_iouring "$server_dir"
            fi
            ;;
        nginx)
...
python3 "${ROOT_DIR}/benchmarks/wrk/analyze_wrk_compare.py" "$RESULTS_DIR"
