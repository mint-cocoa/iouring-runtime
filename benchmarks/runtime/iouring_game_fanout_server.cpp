...
public:
    FanoutSession(int fd, iouring::event::IoRing& ring,
...
int main() {
    ...
    iouring::event::WorkerSessionFactoryBuilder factory_builder =
        [hub](iouring::core::ContextId) {
        return [hub](int fd, iouring::event::IoRing& ring,
                     iouring::core::buffer::BufferPool& pool,
    ...
    iouring::event::WorkerPoolConfig config;
    config.address = iouring::core::Address{"0.0.0.0", port};
    ...
    iouring::event::WorkerPool workers(config, std::move(factory_builder));
    if (!workers.Start()) {
