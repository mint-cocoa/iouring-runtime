...
bool Worker::Start() {
    ...
    auto ring_result = event::IoRing::Create(config_.ring);
    if (!ring_result) {
...
void Worker::Run() {
    ...
    event::IoRing::SetCurrent(ring_.get());
    if (hooks_.on_start) {
    ...
    event::IoRing::SetCurrent(nullptr);
}

std::shared_ptr<net::Listener> Worker::MakeListener(
    ...
    SessionFactory wrapped_factory =
        [this, factory = std::move(factory)](
            int fd, event::IoRing& ring_ref, buffer::BufferPool& pool_ref,
