...
class PlainProxyPeerSession final : public net::Session, public ProxyPeer {
public:
    PlainProxyPeerSession(int fd, event::IoRing& ring,
    ...
    bool SendProxyPayload(std::span<const std::byte> data) final {
        ...
        if (!send_result) {
            obs::LogWarn(kLogCategory, "TcpProxyServer: plain proxy Send() failed");
            return false;
        }
        return true;
    }
...
class TlsProxyPeerSession final : public net::Session, public ProxyPeer {
public:
    TlsProxyPeerSession(int fd, event::IoRing& ring,
                        core::buffer::BufferPool& pool,
                        std::shared_ptr<ProxyBridge> bridge,
    ...
    bool SendProxyPayload(std::span<const std::byte> data) final {
        if (!ssl_ || Disconnecting()) {
            obs::LogWarn(kLogCategory, "TcpProxyServer: TLS proxy cannot send payload while disconnected");
            return false;
        }
...
private:
    void InitializeSsl() {
        if (!tls_context_ || !tls_context_->ctx) {
            obs::LogError(kLogCategory, "TcpProxyServer: downstream TLS context is not initialized");
            return;
        }
    ...
    bool QueueCiphertext(std::size_t bytes) {
        ...
        if (!Send(std::move(*buffer_result))) {
            obs::LogWarn(kLogCategory, "TcpProxyServer: TLS ciphertext Send() failed");
            return false;
        }
        return true;
    }
...
net::SessionRef CreatePlainProxySession(
...
net::SessionRef CreateTlsProxySession(
    int fd, event::IoRing& ring, core::buffer::BufferPool& pool,
    std::shared_ptr<ProxyBridge> bridge,
    std::shared_ptr<DownstreamTlsContext> tls_context,
    DownstreamTlsReadyCallback on_ready,
...
std::shared_ptr<ProxyPeer> ToProxyPeer(const net::SessionRef& session) {
